...
type
  ...
  TRasConnArray = array of TRasConn;
  ...
  public
    ...
    property ActiveEntries: TRasConnArray read FActiveEntries;
    ...
    function GetRasconnOfEntry(EntryName: string): Integer;
    function EntryExists(EntryName: string): Boolean;
...
begin
  ...
  if Result_=0 then
  begin
    SetLength(FActiveEntries, NumEntries);

    if NumEntries > 0 then
    begin
      for X:=1 to NumEntries do
      begin
        FActiveEntries[X-1].dwSize := Entries[X].dwSize;
        FActiveEntries[X-1].hrasconn := Entries[X].hrasconn;
        StrCopy(FActiveEntries[X-1].szEntryName, Entries[X].szEntryName);
        StrCopy(FActiveEntries[X-1].szDeviceType, Entries[X].szDeviceType);
        StrCopy(FActiveEntries[X-1].szDeviceName, Entries[X].szDeviceName);
...
function TDialUp.GetRasconnOfEntry(EntryName: string): Integer;
var
  i: Integer;
begin
  Result := 0;
  GetConnections;
  for i:=0 to High(FActiveEntries) do
  begin
    if StrPas(FActiveEntries[i].szEntryName) = EntryName then
    begin
      Result := FActiveEntries[i].hrasconn;
      break;
    end;
  end;
...
function TDialUp.EntryExists(EntryName: string): Boolean;
begin
  GetEntries;
  Result := FEntries.IndexOf(EntryName) <> -1;
end;
