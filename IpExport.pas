...
type
  ...
  _IP_ADAPTER_INDEX_MAP = record {存储和适配器名称关联的接口序号}
    Index: ULONG; //和适配器关联的接口序号
    Name: array [0..MAX_ADAPTER_NAME - 1] of WCHAR; //指向一个包含了适配器名称的Unicode字符串
  end;
  ...
  _IP_INTERFACE_INFO = record  {包含本机系统网络适配器列表}
    NumAdapters: Longint; //适配器编号
